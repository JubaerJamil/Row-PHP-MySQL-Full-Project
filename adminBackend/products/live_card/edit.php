                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <?php
                            $id = $_GET['id'];
                            $editQuery = "SELECT * FROM product_card WHERE ID= $id";
                            $editQuery_run = mysqli_query($connection, $editQuery);

                            if (mysqli_num_rows($editQuery_run) > 0) {
                                foreach ($editQuery_run as $item) {
                                    ?>
                                    <form action="../../controller/product_card.php" method="POST">
                                        <div class="col-lg-12">
                                            <input type="hidden" name="id" value="<?php echo $item['id']; ?>">
                                            <div class="mb-3">
                                                <label for="productName" class="form-label">Card Information</label>
                                                <input type="text" class="form-control" name="info" id="" value="<?php echo $item['card_info']; ?>" placeholder="">
                                            </div>
                                        </div>
